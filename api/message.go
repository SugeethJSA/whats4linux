...
// prepareOutgoing builds the waE2E.Message for a send request, decoding the
// base64 payload (if any) and reporting the media type needed for upload.
// Pure helper — no network — so the per-type routing is unit-testable.
func prepareOutgoing(content MessageContent, contextInfo *waE2E.ContextInfo) (msg *waE2E.Message, data []byte, mediaType whatsmeow.MediaType, label string, err error) {
	withContext := func(contextInfo *waE2E.ContextInfo, mentions []string) *waE2E.ContextInfo {
		if contextInfo == nil {
			contextInfo = &waE2E.ContextInfo{}
		}
		if len(mentions) > 0 {
		...
		if len(content.Mentions) > 0 || contextInfo != nil {
			return &waE2E.Message{
				ExtendedTextMessage: &waE2E.ExtendedTextMessage{
					Text:        &content.Text,
					ContextInfo: withContext(contextInfo, content.Mentions),
				},
			}, nil, "", "", nil
		}
		return &waE2E.Message{Conversation: &content.Text}, nil, "", "", nil
	case "image":
		imageData, err := base64.StdEncoding.DecodeString(content.Base64Data)
		if err != nil {
			return nil, nil, "", "", fmt.Errorf("failed to decode base64 image data: %v", err)
		}
		...
		if len(content.Mentions) > 0 || contextInfo != nil {
			imageMsg.ContextInfo = withContext(contextInfo, content.Mentions)
		}
		return &waE2E.Message{ImageMessage: imageMsg}, imageData, whatsmeow.MediaImage, "image", nil
	case "video":
		videoData, err := base64.StdEncoding.DecodeString(content.Base64Data)
		if err != nil {
			return nil, nil, "", "", fmt.Errorf("failed to decode base64 video data: %v", err)
		}
		mimeType := content.Mimetype
		if mimeType == "" {
			mimeType = "video/mp4"
		...
		if len(content.Mentions) > 0 || contextInfo != nil {
			videoMsg.ContextInfo = withContext(contextInfo, content.Mentions)
		}
		return &waE2E.Message{VideoMessage: videoMsg}, videoData, whatsmeow.MediaVideo, "video", nil
	case "audio":
		audioData, err := base64.StdEncoding.DecodeString(content.Base64Data)
		if err != nil {
			return nil, nil, "", "", fmt.Errorf("failed to decode base64 audio data: %v", err)
		}
		mimeType := content.Mimetype
		if mimeType == "" {
			mimeType = "audio/ogg"
		...
		return &waE2E.Message{AudioMessage: audioMsg}, audioData, whatsmeow.MediaAudio, "audio", nil
	case "document":
		documentData, err := base64.StdEncoding.DecodeString(content.Base64Data)
		if err != nil {
			return nil, nil, "", "", fmt.Errorf("failed to decode base64 document data: %v", err)
		}
		mimeType := content.Mimetype
		if mimeType == "" {
		...
		if len(content.Mentions) > 0 || contextInfo != nil {
			documentMsg.ContextInfo = withContext(contextInfo, content.Mentions)
		}
		return &waE2E.Message{DocumentMessage: documentMsg}, documentData, whatsmeow.MediaDocument, "document", nil
	case "sticker":
		stickerData, err := base64.StdEncoding.DecodeString(content.Base64Data)
		if err != nil {
			return nil, nil, "", "", fmt.Errorf("failed to decode base64 sticker data: %v", err)
		}
		mimeType := content.Mimetype
		if mimeType == "" {
			mimeType = "image/webp"
		...
		return &waE2E.Message{StickerMessage: stickerMsg}, stickerData, whatsmeow.MediaImage, "sticker", nil
	default:
		return nil, nil, "", "", fmt.Errorf("unsupported message type: %s", content.Type)
	}
}

// attachUploadedMedia fills the media URL/keys on an outgoing message with the
// upload response. The field set is identical for every media variant.
func attachUploadedMedia(msg *waE2E.Message, uploaded whatsmeow.UploadResponse) {
	attach := func(m *waE2E.ImageMessage) {
		m.URL = &uploaded.URL
		m.DirectPath = &uploaded.DirectPath
		m.MediaKey = uploaded.MediaKey
		m.FileEncSHA256 = uploaded.FileEncSHA256
		m.FileSHA256 = uploaded.FileSHA256
		m.FileLength = &uploaded.FileLength
	}
	switch {
	case msg.GetImageMessage() != nil:
		attach(msg.ImageMessage)
	case msg.GetVideoMessage() != nil:
		video := msg.VideoMessage
		video.URL = &uploaded.URL
		video.DirectPath = &uploaded.DirectPath
		video.MediaKey = uploaded.MediaKey
		video.FileEncSHA256 = uploaded.FileEncSHA256
		video.FileSHA256 = uploaded.FileSHA256
		video.FileLength = &uploaded.FileLength
	case msg.GetAudioMessage() != nil:
		audio := msg.AudioMessage
		audio.URL = &uploaded.URL
		audio.DirectPath = &uploaded.DirectPath
		audio.MediaKey = uploaded.MediaKey
		audio.FileEncSHA256 = uploaded.FileEncSHA256
		audio.FileSHA256 = uploaded.FileSHA256
		audio.FileLength = &uploaded.FileLength
	case msg.GetDocumentMessage() != nil:
		document := msg.DocumentMessage
		document.URL = &uploaded.URL
		document.DirectPath = &uploaded.DirectPath
		document.MediaKey = uploaded.MediaKey
		document.FileEncSHA256 = uploaded.FileEncSHA256
		document.FileSHA256 = uploaded.FileSHA256
		document.FileLength = &uploaded.FileLength
	case msg.GetStickerMessage() != nil:
		sticker := msg.StickerMessage
		sticker.URL = &uploaded.URL
		sticker.DirectPath = &uploaded.DirectPath
		sticker.MediaKey = uploaded.MediaKey
		sticker.FileEncSHA256 = uploaded.FileEncSHA256
		sticker.FileSHA256 = uploaded.FileSHA256
		sticker.FileLength = &uploaded.FileLength
	}
}
...
func (a *Api) SendMessage(chatJID string, content MessageContent) (string, error) {
	...
	msgContent, data, mediaType, label, err := prepareOutgoing(content, contextInfo)
	if err != nil {
		return "", err
	}
	if data != nil {
		...
		if err != nil {
			return "", fmt.Errorf("failed to upload %s: %v", label, err)
		}
		attachUploadedMedia(msgContent, uploaded)
	}
