...
## Phase 7 — Chat surface i18n + app polish

- [x] 7.1 Chat-surface i18n migration (en/es/fr) — ~270 new keys in `lib/i18n.ts` (`common.*`, `chat.sidebar.*`, `chat.list.*`, `chat.context.*`, `chat.welcome.*`, `chat.subscribe.*`, `chat.input.*`, `chat.attach.*`, `chat.location.*`, `msg.type.*`, `chat.info.*` 40+, `menu.*`, `msg.*`, `chat.media.*`, `chat.search.*`, `dialog.*`, `toast.*`); migrated with `useT`: ChatScreen (header titles, tab labels, search placeholders, empty states, context menu, delete confirm `{name}` param, SubscribeChannelDialog, WelcomeScreen, chat-list row previews — `yesterday`/`You: ` prefix/`Community` fallback, `missedCall`/`call`/`reacted` subtitle transforms, `common.you`/`unknown` name fallbacks), MessageSearchScreen (filter chips, placeholders, empty states, media fallbacks, `msg.edited`), ChatInput (placeholder, attach menu, location modal, file/image previews, reply preview fallbacks, only-admins note), MessageItem (edited/empty/save/cancel/join-invite/vote/document/call/forwarded/loading/unknown), ChatInfo (group/contact/channel info, participants/created by, disappearing, mute, clear, block, report, subject/topic editing, invite link, announce/lock/send-messages, member-add/join-approval, join requests, communities, exit/join-by-link), ChatDetail (pinned fallback), MessageMenu, QuotedMessage, MediaContent (alt/aria), and dialogs Forward/CreateGroup/CreateChannel/InviteLink/Poll/PollVote/ContactShare; toasts in App.tsx switched to structured `kind` (`success|error|warning`) via `NotificationKind` in useUIStore (`addNotification(message, kind?)`) so the classifier no longer string-matches English; ContactShareDialog re-migrated after an earlier path-typo write silently missed it; `chat.sidebar.refresh` value corrected to match original "Refresh Chats" text
